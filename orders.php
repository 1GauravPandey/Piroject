...
// SQL query to fetch all orders along with corresponding user email
$sql = "SELECT 
  ...
  <table class="table table-bordered table-striped">
    <thead>
      <tr>
        ...
        <th>Status</th>
        <th>Order Date</th>
        <th>Actions</th> <!-- For accepting or rejecting orders -->
      </tr>
    </thead>
    <tbody>
    <?php if ($result->num_rows > 0): ?>
      ...
      <?php while ($row = $result->fetch_assoc()): ?>
        <tr>
          ...
          <td><?php echo htmlspecialchars($row['name']); ?></td> <!-- Escape output -->
          <td><?php echo htmlspecialchars($row['email']); ?></td>
          <td><?php echo htmlspecialchars($row['phone']); ?></td>
          <td><?php echo nl2br(htmlspecialchars($row['address'])); ?></td> <!-- Preserve new lines -->
          <td><?php echo htmlspecialchars($row['payment_method']); ?></td>
          <td><?php echo number_format($row['total'], 2); ?></td> <!-- Format total to 2 decimals -->
          ...
          <td>
            <!-- If order is pending, show Accept and Reject buttons -->
            ...
            <?php else: ?>
              <!-- Otherwise, show no actions available -->
